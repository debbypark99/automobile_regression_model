...
def preprocess(data, names):
    ...
    for line in lines:
        ...
        data.append(make_float(names, segment[2:]))
        
    return preprocessing.normalize(data), target
    
def main():
    
    data, target = preprocess('imports-85-data.csv', 'imports-85-names.csv')
    x_data, x_test, y_data, y_test = train_test_split(data, target)
    ...
    print("loss: {}".format(np.mean(np.square(prediction - y_test))))
    print("=====================\ntest result\n=====================\n")
